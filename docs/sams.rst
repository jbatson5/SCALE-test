...
*B. T. Rearden, L. M. Petrie, M. A. Jessee, M. L Williams*
...
the calculated value of system responses such and *k*\ :sub:`eff`\ and reaction
rate ratios due to perturbations in constituent cross-section data.
Additionally, the uncertainty in each response due to
cross-section-covariance data can be predicted. SAMS performs
sensitivity calculations using linear perturbation theory and requires
the calculation of the forward and adjoint flux moments. Multigroup
sensitivity coefficients computed by SAMS also include the implicit
effect due to resonance self-shielding calculations. In multigroup mode
SAMS works in conjunction with XSDRNPM, NEWT, KENO V.a or KENO-VI, which
are capable of calculating the desired responses as well as associated
fluxes, flux moments and/or mesh fluxes. The implicit calculations are
...
scatter, capture, and fission cross sections, as well as and χ. The
...
TSUNAMI-2D, TSUNAMI‑3D-K5 and TSUNAMI-3D-K6 SCALE control modules and
...
TSUNAMI-1D, TSUNAMI-2D, TSUNAMI-3D-K5 and TSUNAMI-3D-K6 SCALE control
...
Sensitivity System (FORSS) :cite:`lucius_users_1981,weisbin_application_1978,weisbin_application_1976` The
FORSS system was capable of calculating the sensitivity of the system
...
and the volume of each flux region. Additionally, the neutron
...
characterization data are computed system average quantities such as the
...
FORSS\ :sup:`3` code system for fast reactor applications, with the
addition of the sensitivity of *k*\ :sub:`eff` to the fission energy spectrum
(χ). The sensitivity coefficients produced with these techniques give
...
approach :cite:`oblow_sensitivity_1976,stacey_jr_variational_1972,usachev_perturbation_1964,gandini_generalized_1967`.
...
where
   ...
   *λ* = represents the eigenvalues where the largest eigenvalue is
   1/\ *k*\ :sub:`eff`,
...
where *δA* and *δB* represent small linear perturbations in their
corresponding transport operators and *δλ* represents the resulting
...
where *φ\ †* is the adjoint flux and has a special physical significance
as the “importance” of the particles within the system, and *A\ †* and
*B\ †* are the adjoint operators corresponding to *A* and *B*.

Multiplying :eq:`eq6-3-3` by *φ\ †*, and integrating over all phase space yields

.. math::
  :label: eq6-3-5

  \langle \mathop{\phi }^{\dagger }(A-\lambda B+\delta A-\lambda \delta B-B\delta \lambda -\delta \lambda \delta B){\phi }'\rangle =0 .

Using the property of adjointness (i.e., :math:`\langle \mathop{\phi }^{\dagger }(A -\lambda B){\phi }'\rangle =\langle {\phi }'({{A}^{\dagger }} -{{\lambda }^{\dagger }}{{B}^{\dagger }})\mathop{\phi }^{\dagger }\rangle`) and :eq:`eq6-3-4` to reduce the number
of terms yields

.. math::
  :label: eq6-3-7

  \langle \mathop{\phi }^{\dagger }(\delta A-\lambda \delta B-B\delta \lambda -\delta \lambda \delta B){\phi }'\rangle =0 .

:eq:`eq6-3-7` is further simplified by ignoring the second-order perturbation
term (*δλδB*) and substituting *φ′* with *φ*, indicating that the
...
respect to a macroscopic cross section, Σ, of the transport operator at
some point in phase space *r*, the relative sensitivity of *λ*, becomes
...
Note that since *λ* = 1/\ *k*, then *∂λ/λ* = −\ *∂k /k*, where *k* =
*k*\ :sub:`eff`, the sensitivity of *k* due to a small perturbation in a
macroscopic cross section, Σ, of the transport operator at some point in
phase space can be expressed as
...
Eq.  using the discrete ordinates form of the transport equation. In
...
demonstrated in Ref. 3 that sensitivity coefficients for reaction \ *x*,
...
where
   ...
   :math:`\tilde{\phi }_{g,z}^{\dagger j}`= Legendre order that corresponds to the *j*\ :sup:`th` real valued flux
   moment,

   = *j*\ :sup:`th` component real valued adjoint flux moment for energy group
   *g*, and region *z*,

   *NMOM* = the total number of real valued flux moments corresponding
   to the desired Legendre order of expansion.

   *V\ z =* the volume of region *z*

.. missing the third thing in where list.
...
subscript *z\ m* denotes fluxes computed in mesh *m* of region *z.*
Also, is the volume of mesh *m* in region *z*.

Similarly, for transport calculations using XSDRNPM and NEWT, the flux
product is computed over the computational mesh using Eq.  where *m*
...
..
   ...
   Only the term is used for this class of reactions where is the
   absorption cross section of interest ((n, γ), (n, α), (n, p), etc.)
...
..
   ...
   .. math::
    ...
    \begin{array}{c}
    S_{f, g, z}^{i}=\frac{1}{D}\left[\left(\frac{1}{k} \bar{v}_{g, z}^{i} \Sigma_{f, g, z}^{i} \chi_{g, z}^{i}-\Sigma_{f, g, z}^{i}\right) P_{g, g, z}^{0}+\sum_{g^{\prime}=1 \atop g \neq g^{\prime}}^{G} \frac{1}{k} \bar{v}_{g, z}^{i} \Sigma_{f, g, z}^{i} \chi_{g^{\prime}, z}^{i} P_{g, g^{\prime}, z}^{0}\right. \\
    \left.-\sum_{f, g, z}^{i} \sum_{=1}^{I S C T}(2+1) P_{g, g, z}\right]
    \end{array}

3.  :math:`\bar{v}` Sensitivity

   The :math:`\bar{v}` reaction only requires :math:`T_{2,g,z}^{i}` and can be expressed as
...
The computation of the unconstrained χ reaction sensitivity only
requires :math:`T_{2,g,z}^{i}`, with the *χ* and *νΣ\ f* terms interchanged and is expressed
as
   ...
   was first developed for the SAGEP code :cite:`hara_sagep_1984` and is implemented in
   SAMS as
   ...
   The constrained χ calculation from :eq:`eq6-3-22` is the default for SAMS.
...
..
   ...
   .. math::
    ...
    S_{x, g, z}^{i}=\frac{1}{D}\left(\sum_{\ell=0}^{I S C T}\left(\Sigma_{x, g \rightarrow g, z}^{\ell, i}-(2 \ell+1) \Sigma_{x, g, z}^{i}\right) P_{g, g, z}^{\ell}+\sum_{g^{\prime}=1 \atop g^{\prime} \neq g}^{G} \Sigma_{x, g \rightarrow g^{\prime}, z}^{\ell, i} P_{g, g^{\prime}, z}^{\ell}\right)
...
shielded group-wise macroscopic cross-section data :cite:`williams_eigenvalue_2001`.
...
where *ω\ i* is some input quantity. The *ω\ i* term could represent the
...
of other cross sections must be computed. If *ω\ i* is a certain
...
presented in the example calculation of :cite:`williams_eigenvalue_2001`, as it allows for the
...
to *N* :sup:`i`, the number density of the *i*\ :sup:`th` nuclide. Additionally, the
...
denoted *ω*\ :sub:`m`. The chain rule for derivatives can again be used to
...
.. math::
  ...
  =\sum_{m} \sum_{j} \sum_{y} \sum_{h} S_{k, \Sigma_{y h}^{j}} S_{\sum_{y h}^{j}, \omega_{m}} S_{\omega_{m}, \Sigma_{T}^{i}} S_{\Sigma_{T}^{i}, \Sigma_{T, g}^{i}}=\sum_{m} \sum_{j} \sum_{y} \sum_{h} S_{k, \Sigma_{y, h}^{j}} S_{\Sigma_{y, h}^{j}, \omega_{m}} S_{\omega_{m}, N^{i}} S_{\Sigma_{T}^{i}, \Sigma_{T, 8}^{i}}
  \end{array}
...
implicit quantities computed in :eq:`eq6-3-27` and :eq:`eq6-3-28` with *ω*\ :sub:`m`
varied to
...
results using standard error propagation techniques :cite:`bevington_r1969_nodate`. The forward
...
The nuclear data parameters are represented by the vector **α**, the
...
energy groups. If *M* is the number of nuclide-reaction pairs × the
number of energy groups (i.e., the number of elements in **α**), the
symmetric *M × M* matrix containing the relative variances (diagonal
elements) and relative covariances (off-diagonal elements) in the
nuclear data is **C**\ :sub:`αα`. The elements of **C**\ :sub:`αα` are
...
:cite:`dunn_puff-iii_2000`. SAMS simply reads the covariance data from a standard
COVERX data file.

The vector of length *M* containing sensitivities of the calculated
*k*\ :sub:`eff` to the **α** parameters is represented by **S**\ :math:`_{\bf{k}}`, where each
element is
...
elements of **C\ αα**, where *i, j, x* and *y* are held constant. If *G*
is the number of energy groups, the covariance data for a particular
process is represented as the *G* × *G* matrix :math:`{{\mathbf{C}}_{\alpha _{x}^{i}\alpha _{\mathbf{y}}^{\mathbf{j}}}}` and the group-wise
...
on the data file with i≠j and/or x≠y, then the transpose matrix
...
and TSUNAMI-2D to address more general responses that depend on the
...
:cite:`williams_perturbation_1986`.
...
.. math::
  ...
  R=\frac{\left\langle \Sigma _{c}^{(fertile)}\,\phi  \right\rangle }{\left\langle \Sigma _{a}^{(fissile)}\,\phi  \right\rangle }\ ,

.. math::

  \Rightarrow \ \ \ \ {{H}_{N}}=\Sigma _{c}^{(fertile)}\ \,;\ \,{{H}_{D}}=\Sigma _{a}^{(fissile)}\ ;
...
.. math::
  ...
  R=\frac{\left\langle \frac{h({{\text{V}}_{z}})}{{{\text{V}}_{z}}}\Sigma _{f}^{{}}\,\phi  \right\rangle }{\left\langle \frac{1}{{{\text{V}}_{tot}}}\Sigma _{f}^{{}}\,\phi  \right\rangle }\ ,


.. math::

  \Rightarrow \ \ \ \ {{H}_{N}}=\frac{h({{\text{V}}_{z}})}{{{\text{V}}_{z}}}\Sigma _{f}^{{}}\ \,\,;\ \,\,{{H}_{D}}=\frac{1}{{{\text{V}}_{tot}}}\Sigma _{f}^{{}}\ ;


where

	 :math:`h({{V}_{z}})=1\ ,\ \ r\in {{V}_{z}}`   ;   :math:`h({{V}_{z}})=0\ ,\ \ r\notin {{V}_{z}}\ `
...
.. math::
  ...
  R=\frac{\left\langle \,[1-h(E<{{E}_{th}})\,]\,\phi  \right\rangle }{\left\langle (h(E<{{E}_{th}})\,)\ \phi  \right\rangle }\ ,

.. math::

  \Rightarrow \ \ \ \ {{H}_{N}}=\,\ [1-h(E>{{E}_{th}})]\,\,;\ \,\,{{H}_{D}}=h(E<{{E}_{th}}) ; {{E}_{th}} ≡ \text{thermal energy boundary}.


where

  :math:`h(E<{{E}_{th}})=1\ ,\ \ E<{{E}_{th}}` ; :math:`h(E<{{E}_{th}})=0\ ,\ \ E>{{E}_{th}}`
...
.. math::
  ...
  R=\frac{\left\langle h(E<{{E}_{th}}){{\Sigma }_{a}}\ \phi  \right\rangle }{\left\langle h(E<{{E}_{th}})\,\phi  \right\rangle }\ ,


.. math::

  \Rightarrow \ \ \ \ \ {{H}_{N}}=\ \,{{\Sigma }_{a}}(E)\,h(E<{{E}_{th}})\ \,;\ \,{{H}_{N}}=h(E<{{E}_{th}})\ .
...
calculated ratio-response. Let α be an arbitrary input parameter (number
...
H\ :sub:`D` depend on α, or because the parameter appears in the
transport equation, so that the flux is a function of α; i.e.,
...
The response perturbation due to a sufficiently small variation inα can
...
explicitly (or implicitly through self-shielding) on data parameter α.
...
denominator of :eq:`eq6-3-41`, which is caused by varying α in the transport
equation.

If the response functions depend explicitly on α, then the functional
...
operators *A* and *B*, as well as the flux :math:`\phi` and eigenvalue λ may all be
functions of α. Taking the first-order expansion for each of these
...
.. math::
  ...
  \ \frac{1}{R}\,\,\frac{\partial R}{\partial \phi }\ \quad =\ \ \,\frac{{{H}_{N}}}{\left\langle {{H}_{N}}\,\phi  \right\rangle }\ \ -\ \ \frac{{{H}_{D}}}{\left\langle {{H}_{D}}\,\phi  \right\rangle }\ .
...
.. math::
  ...
  \left\langle \,\left( \frac{\partial R}{\partial \phi }\frac{d\phi }{d\alpha } \right)\ \,\delta \alpha  \right\rangle =
  -\ \left\langle {{\Gamma }^{*}}\ \left( \frac{\partial A}{\partial \alpha }-\lambda \frac{\partial B}{\partial \alpha } \right)\ \phi \,\ \delta \alpha  \right\rangle \ \ +\ \ \delta \lambda \left\langle {{\Gamma }^{*}}B\phi  \right\rangle \ .
...
.. math::
  ...
  \delta {{R}^{(indirect)}}\ \ =\]\[-\ \left\langle {{\Gamma }^{*}}\ \left( \frac{\partial A}{\partial \alpha }-\lambda \frac{\partial B}{\partial \alpha } \right)\ \phi \,\ \delta \alpha  \right\rangle \ .
...
.. math::
  ...
  \delta R_{{}}^{{}}\equiv   \left\{ \frac{R}{\left\langle {{H}_{N}}\,\phi  \right\rangle }\,\,\left\langle \frac{\partial {{H}_{N}}}{\partial \alpha }\ \delta \alpha  \right\rangle \ -\ \frac{R}{\left\langle {{H}_{D}}\,\phi  \right\rangle }\,\,\left\langle \frac{\partial {{H}_{D}}}{\partial \alpha }\ \delta \alpha  \right\rangle  \right\}
  -\ \ \ \left\langle {{\Gamma }^{*}}\ \left( \frac{\partial A}{\partial \alpha }-\lambda \frac{\partial B}{\partial \alpha } \right)\ \phi \,\ \delta \alpha  \right\rangle \ .
...
is singular, since by definition λ is
...
stationary with respect to eigenvalue perturbations introduced by δα.  The
...
The total response perturbation is obtained by summing over the response perturbations due to arbitrary α-variations throughout phase space

 .. math::
  :label: eq6-3-62

  \frac{\delta R}{R}\ \ \cong \ \ \left\langle {{S}_{R,\alpha }}(\overset{\lower0.5em\hbox{$\smash{\scriptscriptstyle\rightharpoonup}$}}{\xi})\\ \frac{\delta\alpha}{\alpha}\left({\overset{\lower0.5em\hbox{$\smash{\scriptscriptstyle\rightharpoonup}$}}{\xi}}\right)\right\rangle\ .
...
impacted by the value of parameter α. This can be treated by extending the
partial derivative operator with respect to αto be a total derivative, so that
...
explicit sensitivities are very similar to those presented in :ref:`6-3-2-1`
...
:ref:`6-3-2-2` for eigenvalue responses is also the same for general responses.
...
calculations will only use the nomix and unconstrainedchi input
...
Cross-Section Evaluation Working Group benchmark specifications :cite:`center_cross_1974`.
...
fractional change in *k*\ :sub:`eff` of 5.7367 × 10\ :sup:`−1` × 0.01 or
5.7367 × 10\ :sup:`−3` (0.57367% ∆k/k). Because the keyword PRTIMP was
...
produce a fractional change in *k*\ :sub:`eff` of 5.6594 × 10\ :sup:`−1` × 0.01
or 5.6594 × 10\ :sup:`−3` (0.56594% ∆k/k). Because the keyword PRTIMP
...
(∆\ *k*\ :sub:`eff`/*k*\ :sub:`eff`\ :sub:` `\ × 100%) due to cross-section-covariance
data is given. Next, a listing of the uncertainty terms as produced by
Eq.  are given. The nuclide-reaction to nuclide‑reaction covariance
matrix responsible for the uncertainty contribution is given followed by
the contribution to the uncertainty in terms of % ∆k/k. The total
...
LEU-COMP-THERM-009 of the ICSBEP Handbook :cite:`committe_international_1999`. The critical assembly
consists of three 8 × 15 fuel-rod arrays separated by 6.46‑mm thick
...
3.2433 × 10\ :sup:`−1` × 0.01 or 3.2433 × 10\ :sup:`−3` (0.32433% ∆k/k).
...
The next output edit contains the uncertainty information. First, the
percent relative standard deviation in *k*\ :sub:`eff`
(∆\ *k*\ :sub:`eff`/*k*\ :sub:`eff` × 100%) due to cross-section-covariance data is
...
contribution to the uncertainty in terms of % ∆k/k. The total
uncertainty can be computed from individual values by adding the square
of the values with positive signs and subtracting the square of the
values with negative signs, then taking the square root. The negative
values are the result of anti-correlations in the
...
.. describe:: SAMS Error 013:  Ending block keyword ____ does not equal the beginning block keyword ____
